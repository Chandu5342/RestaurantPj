...
const SidebarMenuBadge = forwardRef(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      "flex items-center justify-center rounded-full bg-sidebar-accent px-1.5 py-0.5 text-[10px] font-medium text-sidebar-accent-foreground",
      className
    )}
    {...props}
  />
...
const SidebarMenuSubTrigger = forwardRef(
  ({ className, variant = "default", size = "default", asChild = false, tooltip, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    const { isMobile, state } = useSidebar();
    const trigger = (
      <Comp
...
const SidebarMenuSubContent = forwardRef(({ className, ...props }, ref) => (
  <ul ref={ref} data-sidebar="menu-sub-content" className={cn("flex flex-col gap-1", className)} {...props} />
));
SidebarMenuSubContent.displayName = "SidebarMenuSubContent";

// SidebarSkeleton
const SidebarSkeleton = forwardRef(({ className, ...props }, ref) => (
  <Skeleton ref={ref} className={cn("h-8 w-full rounded-md", className)} {...props} />
));
