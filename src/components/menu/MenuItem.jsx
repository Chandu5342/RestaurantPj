...
export const MenuItem = ({ item, onAddToCart }) => {
  return (
    <div className={`card-elevated overflow-hidden ${!item.isAvailable ? "opacity-60" : ""}`}>
      {/* Image */}
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1">
          {item.hasOffer && (
            <Badge variant="offer">{item.offerLabel}</Badge>
          )}
          {item.isNew && (
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mb-3">
          ...
          {Array.from({ length: 3 - item.spicyLevel }).map((_, i) => (
            <Flame key={i} className="w-3 h-3 text-muted" />
          ))}
        </div>
